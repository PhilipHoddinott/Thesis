...
for pp=2:length(tleUP)-1
    sl(pp)=tleUP(pp,1)-tleUP(pp-1,1);
end
sl1=1:length(sl);
figure(3)
plot(sl1,sl)
grid on
for ppp=1:length(tleUP)
    ssl(ppp)=tleUP(ppp,1)-relDeb(ppp,1);
end
...
for ip=1:length(tleUP)
    diff3(ip)=abs(tleUP(ip,1)-relDeb(ip));
end
figure(5)
plot(1:ip,diff3)
grid on
...
figure(6)
hold on
plot(1:ip,tleUP(1:ip,1),'-o')
plot(1:ip,relDeb(1:ip),'-x')
grid on
%{
